...
export const refreshToken = ({ refreshToken }) => {
    return client
        .post(AUTH_REFRESH_ENDPOINT, { refreshToken })
...
export const resetPassword = ({ email }) => {
    return client
        .post(AUTH_RESET_PASSWORD_ENDPOINT, {email})
        .then(response => response.data)
};

export const getInitialData = ({ lang }) => {
    return client
        .get(INITIAL_DATA_ENDPOINT, {  params: { lang } })
        .then(response => response.data);
};
...
export const getUser = ({ userId }) => {
    return client
        .get(`${USER}/${userId}`)
        .then(response => getUserResponseAdapter(response.data))
};

export const setPlaces = ({ userId, places }) => {
    return client
        .put(`${USER}/${userId}${UPDATE_PLACES}`, places)
        .then(response => response.data)
};

export const updateEmail = ({ userId, email, password }) => {
    return client
        .put(`${USER}/${userId}${UPDATE_EMAIL}`, { email, password })
        .then(response => response.data)
};

export const updatePassword = ({ userId, password, newPassword }) => {
    return client
        .put(`${USER}/${userId}${UPDATE_PASSWORD}`, { password, newPassword })
        .then(response => response.data)
};

export const updateUser = ({ userId, firsName, lastName, phone }) => {
    return client
        .put(`${USER}/${userId}`, { firsName, lastName, phone })
        .then(response => response.data)
};
...
export const createObservation = ({ observation }) => {
    return client
        .post(OBSERVATIONS_ENDPOINT, observation)
        .then(response => response.data)
};

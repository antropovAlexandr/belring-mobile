...
const Wizard = ({ children, onSubmit, formProps, renderFooter }) => {
  ...
  const isLastPage = useMemo(() => currentPage === Children.count(children) - 1, [
    currentPage,
    children,
  ]);
  const activePage = useMemo(() => Children.toArray(children)[currentPage], [
    currentPage,
    children,
  ]);
  ...
  return (
    <Form {...formProps} validate={validate} onSubmit={handleSubmit}>

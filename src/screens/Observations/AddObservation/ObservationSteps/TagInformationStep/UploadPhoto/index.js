...
export default ({fields, index, input, onPressPreview}) => {
    ...
    const onShowDeleteModal = useCallback(() => {setIsDelete(true)}, []);
    const onDeletePhoto = useCallback(() => {
        setIsDelete(false);
        onChange(null);
        fields.remove(index);
    }, [fields, index]);
    const onHideDeleteModal = useCallback(() => {setIsDelete(false)}, []);

    return (
        <>
            <View style={styles.container}>
                {(value) ? <ImagePreview source={value} onPress={onPressPreview} onPressDelete={onShowDeleteModal} /> : <ImageUploader onPress={onLoadPhoto}/>}
            </View>
